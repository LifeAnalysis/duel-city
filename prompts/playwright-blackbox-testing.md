...
4. 通过页面上的回放上传入口选择该文件。
5. 点击“开始回放”。
6. 前端连接真实 replay 服务，提交 `.yrp3d` 数据。
7. replay 服务返回 STOC 消息，前端按正常路径处理。
8. 页面跳转到 `/duel`。
9. 断言卡牌 DOM 已渲染，并出现 `Win` 结算结果。
...
当前黑盒测试走真实 `replay.neos.moe` 服务，因此它能覆盖真实联调路径，但也会受网络和服务状态影响。

如果后续需要更稳定的 CI，可以增加第二类测试：

- 黑盒联调测试：使用真实 replay 服务。
- 稳定回归测试：用本地可控服务模拟 replay 服务，但仍只通过浏览器边界驱动页面。
